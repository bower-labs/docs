...
- A **specific value** you state ("we use tip 7", "set temperature to 42 °C")
- A **correction** to the protocol body ("we always do this cold", "skip step 3")
...
You don't have to use special phrasing. The first time you say *"we use tip 7 here"* during step 2 of a protocol, Bird saves that against step 2 of that protocol. The next time you run it, Bird already knows.
...
On the Protocol detail page, open the kebab menu (`⋮`) and choose **Bird AI annotations**. The dialog shows everything Bird has captured for that protocol, grouped by step. Read-only for now; admins will eventually be able to promote a bullet into the canonical protocol body.
