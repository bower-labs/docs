...
Live sessions run on your phone. It's a hands-free, camera-in-hand mode, so the entry points are on mobile — if you're at your desk, the onboarding checklist has a QR code that signs your phone in. With video enabled, Bird can see what your camera sees and respond to visual context: "What's the reading on that display?" or "Does this sample look contaminated to you?" are reasonable things to ask.
...
There are two ways in, both on your phone.
...
<Steps>
  ...
  <Step title="Tap the live button">
    The **live** button sits beside Send in the message box — a waveform icon with a soft glow. It's always there, whether or not you've typed anything.
  </Step>
...
In **Voice** or **Video** mode, tap the **Go live with Bird** card below the capture ring. Starting from Video mode turns the camera on as soon as the session connects, so you can point and narrate straight away. See [Mobile capture](/capture/mobile-capture).
...
Because landing on the wrong step mid-procedure is disruptive, **a jump is confirmed before it happens**. Bird works out which step you meant and reads its title back as a quick check — *"Jump to 'Set up gas lines' — that right?"* — and waits. Say *"yes"* / *"do it"* / *"go ahead"* and Bird makes the jump. Say no, or name a different step, and it simply moves on to that instead — nothing jumps until you confirm.

Marking a step done, skipping, and going back all stay instant — only a jump asks for confirmation. If Bird isn't sure which step you meant, it asks you to say which one rather than guessing.
