...
Workspace instructions are **edited only by workspace owners and admins**. Members and guests see them in read-only mode so they understand what's shaping Bird's behaviour, but can't change them.
...
If your workspace is in Restricted mode (HIPAA-ready), Bird automatically adds a locked compliance section to its system instructions. Workspace instructions must not contain:
