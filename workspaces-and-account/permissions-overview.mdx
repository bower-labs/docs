...
* **Guest** (free) — read-only across the workspace. Right for stakeholders, supervisors, or external collaborators who need to see work but not change it. Capped by plan (Starter 1 / Pro 4 / Team 4 per paid seat).
...
|---|---|---|
| **Anyone in this workspace** *(default)* | all members and guests | paid members at their workspace role; guests never edit |
...
* Workspace Guest opens a default-public note → reads it (Guest can read, entity is open). They cannot edit (Guest role blocks).
