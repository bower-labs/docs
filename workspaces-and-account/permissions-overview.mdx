...
| Ask [Bird](/ai-and-search/bird-ai-assistant) | yes | yes | yes | read-only |
| [Live agent mode](/ai-and-search/live-agent-mode) | yes | yes | yes | no |
| Connect a [source](/integrations/connectors) | yes | yes | yes | no |
...
* **Guest** (free) — read-only across the workspace, no seat cost. Capped by plan (Starter 1 / Individual 4 / Team 4 per paid seat). Right for external collaborators and stakeholders who need visibility but don't warrant a paid seat. Guests can ask Bird about the workspace — read, search and summarise — but Bird won't create or change anything for them. Their chats draw on the workspace's shared [AI allowance](/workspaces-and-account/billing#ai-usage-allowances).
