...
The **Capture** tab on mobile is built around a single capture ring and a four-mode picker — **Voice**, **Photo**, **Video**, **Text**. Pick a mode, capture, review, and save. Everything syncs to your desktop within seconds.
...
<Steps>
  ...
  <Step title="Choose Photo note or Equipment">
    A toggle on the review screen lets you save it as a normal **Photo note**, or as **Equipment** — in which case Bird reads the nameplate and pre-fills make, model, and serial straight into your [Equipment Register](/organisation/equipment-register). If you got here by scanning the QR code from **Add equipment** on desktop, the toggle is already set to Equipment.
  </Step>
...
Recognised equations appear as mathematical notation in the note. Diagrams and other relevant figure regions appear inline as crops from the original photo, in reading order, rather than as AI-redrawn diagrams. If Bower cannot preserve part of the layout or a crop, it flags the note for review. The original photo remains attached as the source of truth.

## Video

Record a clip up to 5 minutes. Bower preserves the original video as an attachment. Video captures are not currently transcribed.
...
In **Voice** and **Video** mode, a **Go live with Bird** card sits below the capture ring. Tap it to start a [live agent session](/ai-and-search/live-agent-mode) instead of a one-off capture — a continuous conversation where you talk through your work and Bird responds as you go.

The difference is what you want back. A capture gives you a note or an attachment; a live session gives you a conversation, and Bird saves things only when you ask it to.

Starting from **Video** mode turns your camera on as soon as the session connects, so you can point and narrate straight away. From **Voice** mode the session starts audio-only, and you can turn the camera on later from the control bar.

<Note>
The card appears in Voice and Video mode only. Photo and Text captures don't have a live equivalent.
</Note>
