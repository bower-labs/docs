---
title: "Connectors"
description: "Bower connects to 23 sources out of the box — 8 always-on scholarly indexes plus 15 of the tools your lab already uses. Federated search by default; nothing lands in your knowledge base until you save it."
---

## 8 always-on sources — no setup

Out of the box, Bower grounds answers in eight open sources with no connection step. The new **citation trail** primitive walks both the references _and_ the forward citations of any paper you give it, then ranks by citation-graph proximity — much stronger relevance than keyword overlap.
...
**Save your ORCID iD on your profile** (Settings → Profile → _Research identity_) and Bower will pull from your publication list when you ask things like _"what have I published recently on coral alkalinity?"_ — no need to share the iD each turn. We only ever read your **public** ORCID profile.

## 15 connectors — one-click OAuth

Connectors extend Bower into the tools you've licensed. Each is connected **per user, per workspace** — your OAuth, your access. A colleague's connections aren't visible from your account.
...
### Start with the tools behind your sign-in

If you signed in to Bower with Google or Microsoft, the **Get set up** section on your dashboard may suggest the tools that match that account:

- **Google:** Google Drive, Google Docs, and Google Sheets.
- **Microsoft:** OneDrive and OneNote.

Choose a suggested tool to start its normal provider authorisation flow. You can still connect any supported tool later from **Account Settings → Connectors**. The suggestion does not share access automatically: each tool needs its own approval.

<Note>
  The dashboard suggestion is not shown in Restricted-mode workspaces, or after you already have an active connector. Connectors remain unavailable in Restricted-mode workspaces.
</Note>
...
Connecting a tool lets Bower _search_ it in place. Importing is different: it makes a copy of your content **inside Bower**, as notes you can edit, search, link, and organize like anything else you've captured. OneNote is the first connector you can import from.
...
- **Re-importing won't duplicate.** Bower recognizes pages it has already imported, so running the same import again won't create duplicate notes.
- **Large notebooks.** A single import handles up to 500 pages. For a bigger notebook, import one section at a time.
- **Availability.** Import is part of connectors, which are available on every plan — Starter, Individual, and Team. Restricted-mode workspaces can't use connectors yet.
...
<AccordionGroup>
  ...
  <Accordion title="Searches respect provider permissions">
    Bower sees only what your account has access to in the provider. If your admin removes your access, Bower loses it on the next search.
  </Accordion>

  <Accordion title="Read-only by default">
    Bower never writes back to your connected tools. Content enters your knowledge base only when you explicitly ask Bower to save a result.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Re-auth — when a tool stops working">
    OAuth tokens expire. When that happens the card shows a **Re-auth** badge and Bower stops using that connector until you reconnect. Click **Reconnect**.
  </Accordion>
  ...
  <Accordion title="Working with binary files (XLSX, PDF, images)">
    Bower returns a clickable link to binary files in storage tools. To analyze the contents, upload the file to your Bower workspace and Bower will read it with its built-in spreadsheet, PDF, and OCR tools.
  </Accordion>
</AccordionGroup>
