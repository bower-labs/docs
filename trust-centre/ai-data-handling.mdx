...
| **OpenAI** (gpt-4o-transcribe) | Voice transcription **fallback** only — used if Google Speech-to-Text is briefly unavailable. Not used in Restricted mode. | US | No |

Most of Bower's AI runs on **Google Vertex AI**, which is covered by a signed Business Associate Agreement (BAA). The other providers handle narrower tasks and are **never used by workspaces in [Restricted mode](/trust-centre/hipaa-compliance)** (see below).
...
If you work with especially sensitive material — protected health information, confidential or unpublished research, trade secrets, regulated data — you can enable **[Restricted mode (HIPAA-ready)](/trust-centre/hipaa-compliance)** on a workspace. It enforces the strictest controls:
