...
- Every database query is scoped to the current workspace by an enforcement layer that sits beneath the application code, rather than being added route by route.
...
This isolation does not depend on a developer remembering to add it: our test suite fails the build if any route that touches workspace data is missing its access check.
...
For exactly which providers we use, where your data goes in transit, how long it's kept, and how to lock it down, see [AI & your data](/trust-centre/ai-data-handling). For the full list of every service that processes your data, see our [sub-processor list](/trust-centre/sub-processors).
